...
@foreach ($categorias as $categoria)
      <a href="{{route('noticias.categoria',$categoria->id)}}"> {{$categoria->nombre}} </a>
@endforeach
<a href="{{route('paginaPrincipal')}}"> Todas </a>
          ...
          <div>
            <div class="shadow-lg hover:shadow-xl transform transition duration-500 hover:scale-105">
              <div>
                ...
                <div class="px-4 py-2">
                  <h1 class="text-xl font-gray-700 font-bold">{{$noticia->titulo}}</h1>
                  <div class="flex space-x-2 mt-2">
                    <h3 class="text-lg text-gray-600 font-semibold mb-2">{{ $categorias[$noticia->categoria_id-1]->nombre }}</h3>
                  </div>
