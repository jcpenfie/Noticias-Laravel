...
@section('header')
    <nav class="bg-yellow-300 border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-800 sticky top-0 z-50">
        <div class="relative container flex flex-wrap justify-between items-center mx-auto">
            <a href="{{ route('paginaPrincipal') }}" class="flex">
                <svg xmlns="http://www.w3.org/2000/svg" class="mr-3 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="hidden w-full md:block md:w-auto" id="mobile-menu">
                <ul class="flex flex-col mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium">
                    @foreach ($categorias as $categoria)
                        <li>
                          <a href="{{ route('noticias.categoria', $categoria->id) }}" class="block py-2 pr-4 pl-3 text-gray-700 hover:bg-gray-50 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent">{{ $categoria->nombre }}</a>
                        </li>
                    @endforeach
                    <li>
                      <a href="{{ route('paginaPrincipal') }}" class="block py-2 pr-4 pl-3 text-gray-700 hover:bg-gray-50 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent">Todas</a>
                    </li>
                </ul>
            </div>
        </div>
